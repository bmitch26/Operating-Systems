...
# from module import *
from sqliteCRUD4 import SqliteCRUD
...
# dataPath = "./data/"
# dbName = "filesystem2.db"
# dbFullPath = os.path.join(dataPath, dbName)

# if os.path.exists(dbFullPath):
#     print(f"Database found at: {dbFullPath}")
#     fsDB = SqliteCRUD(dbFullPath)
#     print("Database connection initialized successfully.")
# else:
#     print(f"Database file not found at: {dbFullPath}")
#     fsDB = None

import os

dataPath = "/home/brtm23/ComputerScience/5143_OperatingSystems/P01FileSystemShell/"
dbName = "filesystem2.db"
dbFullPath = os.path.join(dataPath, dbName)
...
def did(directory_path):
    query = f"SELECT * FROM directories WHERE name = '{directory_path}'"
    response = fsDB.runQuery(query)
    if not response['data']:
        return {"error": "Directory not found"}, 404
    directory = response['data'][0]  # Get the first row
    # return f"dir={directory}"
    return int(directory[0])
...
@app.get("/dir")
def list_directory(directory_path: str="/home", l: bool = Query(False), a: bool = Query(False), h: bool = Query(False)):
    """
    Lists the contents of a directory and logs the access in the database.
    """
    if fsDB is None:
    ...
    try:
        #step 1: query the directories table to find the directory by name
        dir_id = did(directory_path)
        print(dir_id)
        
        # step 2: Query files and directories within this directory
        where = f"WHERE pid LIKE {dir_id}"
        where = "WHERE 1"
        files_query = f"SELECT MIN(pid) FROM files {where} LIMIT 5"
        # pid LIKE {directory_id}
        # directories_query = f"SELECT * FROM directories = '{directory_id}'"
        
        files = fsDB.runQuery(files_query)  # Fetch all files
        
        return files
        directories = fsDB.runQuery(directories_query)['data']  # Fetch all directories

        #step 3: filter out hidden files/directories if the 'a' flag is not set
        # if not a:
    ...
    #     #step 4: iterate over files and directories, applying the flags
    #     for item in files + directories:
    #         if 'size' in item:
    #             #this is a file
    #             if l:
    #                 #gather detailed file info (permissions, size, and timestamps)
    #                 size = item['size']
    #                 if h:
    ...
    #                     "permissions": f"r: {item['read_permission']}, w: {item['write_permission']}, x: {item['execute_permission']}"
    #                 })
    #             else:
    #                 directory_info.append(item['name'])
    #         else:
    #             #this is a directory
    #             directory_info.append(item['name'])

    #     # log the action in the database
    #     log_query = f"INSERT INTO directory_logs (directory_path, action, timestamp) VALUES ('{directory_path}', 'listed', '{datetime.now()}')"
    #     fsDB.runQuery(log_query)

        # return {"directory_contents": directory_info}
...
if __name__ == "__main__":
    uvicorn.run("api3:app", host="127.0.0.1", port=8080, log_level="debug", reload=True)