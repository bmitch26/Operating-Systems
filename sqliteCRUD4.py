...
class SqliteCRUD:
    ...
    def describeTable(self, table_name, raw=False):
        ...
        if not raw:
            table = self.__formattedResults(results)
...
if __name__ == "__main__":
    ...
    db_name = "filesystem2.db"
    conn = SqliteCRUD(db_name)

    query = "SELECT * FROM directories WHERE 'name' LIKE '%python%';"
    query = "SELECT * FROM directories WHERE pid = '0';"
    query = "UPDATE directories SET name = 'home' WHERE id = 1;"
    query = "UPDATE directories SET pid = '1' WHERE id = 2;"
    result = conn.runQuery(query)
    query = "SELECT * FROM directories WHERE 1;"
    result = conn.runQuery(query)
    print(result)
    
    # print(conn.describeTable("directories"))
